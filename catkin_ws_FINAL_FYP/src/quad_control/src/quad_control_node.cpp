...
class TopControl
{
  ...
  private:
	...
	float r_vel_estimate, past_r_vel_estimate;

  public:
    TopControl() : t_node_handle_(),
		   rate_(50),
		   pitchPosition(1, 0, 0), //P controller with kp=1
		   pitchVelocity(1, 0, 0),
		   pitchV_estimate(5),
		   rollPosition(0.8,0,0), //P controller with kp=1
		   rollVelocity(3, 0, 0),
		   rollV_estimate(5),
...
/*
    ...
    std::cout << "Dist_y  " <<dist.position.y << std::endl;
    std::cout << "Controller (1) output is: " <<roll_control_op << std::endl;
    //Find the roll rate command

	r_vel_estimate = 0.5 * rollV_estimate.computeDerivative((dist.position.y));

	if (r_vel_estimate > 30) {
		r_vel_estimate = past_r_vel_estimate;
	}
	...
	past_r_vel_estimate = r_vel_estimate;

    output_roll_angle = rollVelocity.computeOutput(roll_control_op,r_vel_estimate);
   std::cout << "Vel estimate: : " << (10*r_vel_estimate) << std::endl;
	std::cout << "Controller (2) output angle is: " <<output_roll_angle << std::endl;
  	//output_roll_angle = roll_control_op;
    ...
    output_roll_angle = output_roll_angle * PI /180;
    // output_yaw_angle = current_yaw;

	//rate_pitch = output_pitch_angle;
	rate_roll = output_roll_angle;


    std::cout << "Desired corrected roll angle is: " << rate_roll << std::endl;
...
int main(int argc, char **argv)
{
    ...
    //Set quad control Parameters

    if(argc == 3){
        myController.rollPosition.setKp(std::atof(argv[1]));
        myController.rollVelocity.setKp(std::atof(argv[2]));
    } 

    if(argc == 5){
        myController.rollPosition.setKp(std::atof(argv[1]));
        myController.rollVelocity.setKp(std::atof(argv[2]));
        myController.rollPosition.setKi(std::atof(argv[3]));
        myController.rollVelocity.setKi(std::atof(argv[4]));
    }   
    ...
    if(argc == 7){
        ...
        myController.rollPosition.setKi(std::atof(argv[3]));
        myController.rollVelocity.setKi(std::atof(argv[4]));
        myController.rollPosition.setKd(std::atof(argv[5]));
        myController.rollVelocity.setKd(std::atof(argv[6]));
    }   

    std::cout << "******Position loop is: ******" << std::endl;
    myController.rollPosition.printGains();

    std::cout << "******Velocity loop is: ******" << std::endl;
    myController.rollVelocity.printGains();
    ...
    ros::Subscriber cam_sub = my_node_handle_.subscribe("distance", 5 , &TopControl::reControl, &myController);
