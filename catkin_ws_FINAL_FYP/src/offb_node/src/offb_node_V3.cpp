...
void Receiver::stateCallback(const mavros_msgs::State::ConstPtr& msg)
{
    ...
    if(state_check == PRELIMINARY_CHECK)
    {
        ...
        else 
        {
			...
			system("rosrun mavros mavsys mode -c OFFBOARD");
        }
        /*if(msg->armed)
        {
...
void Receiver::rcCallback(const mavros_msgs::RCIn::ConstPtr& msg)
{
    ...
    if(msg->channels[LEFT_TRIGGER] > 1700) terminate = true;
...
int main(int argc, char **argv)
{
	...
	if(!receiver.terminate){
		...
		receiver.state_check = MANUAL_CHECK;
	}
	receiver.state_finished = false; //Calls State Callback
	while ((!receiver.state_finished) && (ros::ok()) && (!receiver.terminate))
		ros::spinOnce();

	//ROS_INFO("Commencing: Autonomous control");

	for (int i = 0; i < 8; i++) rc_command.channels[i] = 0; //65535;//Releases all Channels First

	if(!receiver.terminate) {
		//rc_command.channels[THROTTLE] = MIDDLE; //Ascending Throttle
		rc_command.channels[PITCH] = RISING;
